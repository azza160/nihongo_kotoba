...
@section('content')
    <div class="flex justify-center items-center min-h-screen bg-zinc-100 px-4">
        <div class="w-full max-w-md bg-white p-8 rounded-xl shadow-md">
            {{-- Judul --}}
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Masuk ke Kotoba</h1>

            {{-- Deskripsi Santai --}}
            <p class="text-sm text-gray-600 mb-6 text-justify">
                Selamat datang kembali! Yuk lanjutkan petualanganmu mempelajari kosakata Bahasa Jepang.
            </p>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <div class="relative">
                    <input type="text" id="nama_pengguna" name="nama_pengguna" value="{{ old('nama_pengguna') }}"
                        class="block px-2.5 pb-1.5 pt-3 w-full text-sm text-gray-900 bg-transparent rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 
                    focus:border-blue-600 peer @error('nama_pengguna') border-red-500 @enderror"
                        placeholder=" " required />
                    <label for="nama_pengguna"
                        class="absolute text-sm text-gray-500 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 
                ...
                <div class="relative">
                    ...
                    <label for="kata_sandi"
                        class="absolute text-sm text-gray-500 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 start-1">Kata
                        Sandi</label>
                    <button type="button"
                ...
                {{-- Tombol --}}
                <button type="submit"
                    class="w-full py-2.5 px-4 text-white bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-medium transition">Masuk</button>
                ...
                <p class="text-center text-xs text-gray-500 mt-4">Belum punya akun? <a href="{{ route('register') }}"
    ...
    @if (@session('register_success'))
        <script>
            Swal.fire({
                ...
                text: 'Selamat akun kamu berhasil dibuat. Silakan login.',
                confirmButtonColor: '#3B82F6',
            });
        </script>
    @endif
